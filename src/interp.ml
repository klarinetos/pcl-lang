(* Tree-walking interpreter for PCL. This is the second interpreter (see
   guide/INTERP_WALKTHROUGH.md for how this branch relates to the first,
   `interpreter-phase1-2`) - same design, ported to the Phase 3 AST shape
   (stmt/header/var_group now carry line numbers) and, unlike the first one,
   run *after* Semantic.check_program has passed (see interp_main.ml), so it
   no longer has to assume the program is well-formed on faith. *)
...
(* Assignment-compatibility coercion (SPEC.md §7): integer widens to real
   when the target expects real. Everything else must already match - by
   the time this runs, Semantic.check_program has already rejected any
   program where that wouldn't hold (see interp_main.ml), so this is a
   straight coercion, not a check. *)
(* ------------------------------------------------------------------ *)
...
(* type_of_lvalue / type_of_expr: a small, partial type inference, only as
   ...
   type-directed). This still assumes the program is well-formed and does
   no checking of its own - it is not a second copy of Semantic, just a
   lookup helper - but by the time it runs, Semantic.check_program (a real
   check, unlike this) has already verified that assumption. *)
(* ------------------------------------------------------------------ *)
...
and eval_expr (env : env) (e : expr) : value =
  ...
  (* and/or short-circuit per SPEC.md §5.3: the second operand is not
...
(* Arithmetic result-type rules per SPEC.md §5.3: +,-,* stay integer
    ...
    (fun loc ->
      ...
      | LVar groups ->
          List.iter
            (fun g ->
              List.iter
                (fun name -> Hashtbl.replace env.vars name (ref (default_value g.vtyp), g.vtyp))
                g.vnames)
            groups
      | LLabel (_, _) -> () (* goto/label resolution happens structurally in exec_stmts, no entry needed *)
...
and exec_stmts (env : env) (stmts : stmt list) : unit =
  ...
  let find_label label =
    let rec go i =
      if i >= n then None
      else match arr.(i).sdesc with SLabel (l, _) when l = label -> Some i | _ -> go (i + 1)
    in
    go 0
  in
  let rec run i =
